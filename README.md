...
Python, LangGraph, FastAPI, ChromaDB, QLoRA, vLLM, Docker, AWS EC2
...
Python, LangGraph, LangSmith, FastAPI
...
- LLM: LangGraph, LangChain, LangSmith, ChromaDB
- 모델: Hugging Face, QLoRA, vLLM
- 서버와 배포: FastAPI, SQLAlchemy, Docker, AWS EC2, GitHub Actions

<br/>

## 일하는 방식

코드 작성은 AI의 도움을 많이 받는다. 다만 실행하고, 에러 로그를 읽고, 왜 이런 결과가 나왔는지 하나씩 확인하는 과정은 직접 한다. 가장 오래 걸리는 건 에러 메시지가 진짜 원인을 알려주지 않을 때라서, print로 중간값을 찍고 캐시를 지우며 재현하는 걸 반복해 원인을 좁혀 간다. 붙여넣은 코드가 개행 없이 저장된 걸 겪은 뒤로는 파일을 고치면 `wc -l`로 줄 수부터 확인한다.
